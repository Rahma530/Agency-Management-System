-- ============================================================================
-- CANONICAL SCHEMA v4 — full clean rebuild, superseding
-- 20260926100000_canonical_schema_v3.sql AND
-- 20260927100000_employee_client_brief_management.sql (both fully folded
-- in below; that migration file's changes are no longer applied on top of
-- v3 — this file supersedes both).
-- ============================================================================
--
-- WHY THIS FILE EXISTS
-- --------------------
-- The live database is STILL confirmed empty of real employee/client data,
-- so this is one more from-scratch rebuild instead of layering the
-- employee/client/brief-management migration on top of v3. THE "LAST TIME"
-- WARNING FROM v2 AND v3 STILL APPLIES, MORE THAN EVER: once real employee/
-- client data exists, all future schema changes must be additive migrations
-- only (never DROP TABLE, never a wipe-and-rebuild like this one). Ask
-- again before another one of these.
--
-- SOURCE OF TRUTH
-- ----------------
...
--
-- WHAT'S NEW IN v4 (consolidated from
-- 20260927100000_employee_client_brief_management.sql, no migration file
-- ever run against a live DB for any of this — held pending this rebuild)
-- --------------------------------------------------------------------------
-- 1. Employee deactivation (NOT hard delete). users.deactivated_at
--    (nullable timestamptz) marks an employee as permanently shut off from
--    new work while their row — and therefore their name on every
--    historical task/brief/daily-log/report — stays intact. Their
--    auth.users account is separately BANNED (~100 year ban_duration,
--    reversible) by scripts/deactivateAuthUser.ts, out-of-band, same
--    service-role-only pattern as scripts/provisionAuthUsers.ts — nothing
--    in this file performs that ban itself.
--    Edit access (name/email/role/team/capacity_limit) and deactivate
--    access are both scoped to executive/head_of_technical + all 5 team
--    leads (am_team_lead, seo_team_lead, media_buying_team_lead,
--    social_media_team_lead) via column-scoped grants — not a blanket
--    UPDATE — same discipline the original capacity_limit-only grant used.
-- 2. Client hard DELETE. clients_delete_rls, scoped to the same 6 roles.
--    No cascade, no reassignment: every one of the 13 tables that
--    reference clients(id) still defaults to ON DELETE NO ACTION, so a
--    client with any activity anywhere simply fails to delete with a plain
--    FK-violation error. Only a client with zero rows in every referencing
--    table can actually be removed. Intentional and final.
-- 3. Briefs:
--    a. briefs_update_rls no longer pins WITH CHECK to
--       `submitted_by = app_user_id()` — every role already listed in its
--       USING clause can genuinely edit an existing brief's answers, not
--       just re-save their own. executive/head_of_technical are also now
--       included (previously absent from this policy entirely).
--    b. No brief DELETE policy — whole-brief deletion was explicitly
--       ruled out. brief_revisions' own append-only logging on every save
--       remains the audit trail.
--    c. briefs.custom_field_defs (jsonb): one-off custom questions added
--       to a SPECIFIC client's brief — lives on the briefs row itself
--       (there is already exactly one live briefs row per (client_id,
--       service_type), so a custom question's scope is already exactly
--       this row's identity). The question's ANSWER needs no new storage —
--       it's just another key in the existing `fields` jsonb column.
--    d. brief_field_schemas (new table): the GLOBAL, per-service-type
--       question list — moved out of the old static
--       src/data/briefFieldSchemas.ts object so it's editable from the app
--       (BriefFieldSchemaEditor.tsx) instead of requiring a code change +
--       redeploy. Seeded below with that file's former content. Write
--       access: executive/head_of_technical/am_team_lead/am_agent
--       unconditionally (this branch already covers `creative`, since
--       there is no dedicated "creative team lead" role in this app —
--       creative resources are pooled under am_team_lead's oversight
...
-- phone_number, packages dropped) and v2's header before that for the
-- original RLS/function decisions (task_visible's 3-arg signature,
-- direct_report_visible's narrowed HR-data visibility, the
-- client_comparisons/reports 8-role dispatch, users.auth_id nullability,
-- etc). Section numbers below intentionally skip "1.2 packages" and
-- "4.2 packages" (removed in v3) rather than renumbering every subsequent
-- section — pure cosmetics, not worth the transcription risk in a file
-- this size for a from-scratch review.
-- ============================================================================
...
--    too, not just an empty one. packages stays dropped (see v3's header,
--    "ON DROPPING packages") — not recreated here.
-- ============================================================================
...
create table public.users (
  ...
  -- v4: null while active. Set the moment an executive/head_of_technical/
  -- team-lead deactivates this employee — the row is never deleted, but
  -- they lose all capability (see header, point 1).
...
-- ----------------------------------------------------------------------------
-- 1.2 packages — REMOVED in v3. See v3's header, "ON DROPPING packages".
-- ----------------------------------------------------------------------------
...
create table public.briefs (
  ...
  -- v4: one-off custom questions added to THIS client's brief only — never
  -- touch the global per-service question list in brief_field_schemas
  -- below. The answer to a custom question lives in `fields` exactly like
  -- a built-in field's; only the question definition itself lives here.
...
-- ----------------------------------------------------------------------------
-- 1.24 brief_field_schemas (new in v4) — the GLOBAL, per-service-type brief
-- question list. See header, point 3d.
-- ----------------------------------------------------------------------------
...
-- v4: name/email/role/team, scoped to exec/HoT + all 5 team leads (team
-- leads limited to employees employee_visible() already scopes them to —
-- their own department). Same USING/WITH CHECK predicate as
-- users_update_capacity_rls above and users_deactivate_rls below — kept as
-- 3 separate policies purely for readability, so a future narrowing of one
...
-- v4: deactivate access (sets deactivated_at) — same role scope as above.
...
-- action, not a team-lead one — narrower than the update policies' role
-- set on purpose). `auth_id is null` is load-bearing: it makes it
...
-- ----------------------------------------------------------------------------
-- 4.2 packages — REMOVED in v3. See v3's header, "ON DROPPING packages".
-- ----------------------------------------------------------------------------
...
-- v4: real hard DELETE, scoped to executive/head_of_technical + all 5 team
-- leads. No cascade anywhere in this file, so this only ever succeeds for a
-- client with zero rows in every one of the 13 tables that reference
-- clients(id) — everything else defaults to ON DELETE NO ACTION.
...
-- v4: broadened — no longer pins WITH CHECK to submitted_by = app_user_id(),
-- so every role in the USING clause can genuinely edit an existing brief's
-- answers, not just re-save their own. executive/head_of_technical added
-- (previously absent from this policy entirely).
...
-- ----------------------------------------------------------------------------
-- 4.24 brief_field_schemas (new in v4)
-- ----------------------------------------------------------------------------
alter table public.brief_field_schemas enable row level security;

-- Open to every authenticated user — anyone filling out or viewing a brief
-- needs to see the current question list.
create policy "brief_field_schemas_select_rls" on public.brief_field_schemas
for select to authenticated
using (true);

-- executive/head_of_technical/am_team_lead/am_agent unconditionally (this
-- branch already covers 'creative' — no dedicated "creative team lead"
-- role exists in this app), plus each department team lead scoped to only
...
-- Seed data — the exact content of src/data/briefFieldSchemas.ts's former
-- BRIEF_FIELD_SCHEMAS object at the time it was removed from the frontend
-- bundle, one row per field, sort_order matching each field's original
-- position in its service's array.
