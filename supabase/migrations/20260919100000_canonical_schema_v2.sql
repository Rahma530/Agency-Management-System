-- ============================================================================
-- CANONICAL SCHEMA v2 — full clean rebuild, superseding both
-- 20260916100000_canonical_schema.sql (the first canonical rebuild) and
-- every incremental migration layered on top of it since:
--   20260917100000_employee_provisioning.sql
--   20260918100000_programming_agent_role.sql
--   20260919100000_daily_logs_client_id.sql
--   20260920100000_task_visible_am_agent_client_scope.sql
--   20260921100000_assignment_notifications.sql
--   20260922100000_client_contracts.sql
--   20260923100000_client_payment_tracking.sql
--   20260924100000_task_drive_link.sql
-- ============================================================================
--
-- WHY THIS FILE EXISTS
-- --------------------
-- The live database is still confirmed empty of real employee/client data —
-- the employee-provisioning script has not been run yet — so this is one
-- more from-scratch rebuild instead of applying 8 more patch files in
-- sequence. THIS IS THE LAST TIME: once real employee/client data exists,
-- all future schema changes must be additive migrations only (never DROP
-- TABLE, never a wipe-and-rebuild like this one).
--
-- SOURCE OF TRUTH
-- ----------------
...
--
-- A REAL BUG THIS VERSION FIXES (found while consolidating, not present in
-- the original ask)
-- --------------------------------------------------------------------------
-- v1 declared `users.auth_id` as `not null`, but 20260917's
-- users_insert_admin_rls policy explicitly requires `with check (auth_id is
-- null)` for the "Add Employee" invite-then-claim flow, and
-- UserRecord.auth_id in types/database.ts is `string | null`. No migration
-- ever actually dropped the NOT NULL constraint, so that INSERT policy was
-- structurally impossible to satisfy — the Add Employee feature would have
-- been rejected at the database level the moment it was actually run
-- against a real schema. Fixed here: `auth_id` is now nullable (UNIQUE is
-- kept — Postgres treats every NULL as distinct under a unique constraint,
-- so multiple pending employees can coexist, exactly like
-- client_portal_users.auth_id already does for the client-portal invite
-- flow this pattern was copied from).
--
-- CONFLICTS FOUND IN THE MIGRATION HISTORY, AND HOW THEY'RE RESOLVED HERE
-- -------------------------------------------------------------------------
-- Only the FINAL version of anything rewritten along the way survives below.
-- Carried forward unchanged from v1's own header (still accurate):
--   - task_visible(): 1-arg, then 1-arg (creative-pool scoping), then 2-arg
--     recursive (subtask inheritance). NOW widened again to 3-arg
--     (p_client_id) by 20260920 for am_agent's cross-department client
--     visibility. Final: 3-arg, p_parent_task_id and p_client_id both
--     defaulting to null so every existing 1/2-arg call site still compiles.
--   - daily_logs_select_rls / extra_notes_select_rls /
--     performance_reviews_select_rls / kpi_scores_select_rls: narrowed from
--     employee_visible_by_id() to direct_report_visible() — a genuine
--     security fix (closed a peer-read leak on personal/HR data). Final:
--     direct_report_visible()-based.
--   - briefs_write_rls / briefs_update_rls: widened to let am_team_lead/
--     am_agent author briefs from the client meeting. Final: widened.
--   - client_comparisons_*/reports_* policies: widened to the full 8-role
--     dispatch via report_scope_accessible()/comparison_scope_accessible(),
--     reports_insert_rls admits both 'internal' and 'client' types, and
--     reports_update_rls exists (draft/approval phase). Final: all of that.
--   - packages_select_all_employees: widened to also admit client-portal
--     sessions. Final: widened.
-- New in this version (Module 12 + employee provisioning):
--   - employee_visible() / direct_report_visible(): both gained a
--     programming_agent branch under seo_team_lead's oversight (20260918).
--   - task_visible(): gained the am_agent + p_client_id branch (20260920);
--     tasks_select_rls now passes client_id as the 3rd argument.
--   - assignments_update_rls: widened so the assigned agent (not just their
--     team lead) can write their own row's viewed_at (20260921).
--   - users: users_email_unique constraint + users_insert_admin_rls policy
--     for the Add Employee flow (20260917).
--
-- CHANGES v1 MADE BEYOND WHAT ANY MIGRATION HAD ESTABLISHED AT THE TIME
-- (still true here; see v1's own history for the reasoning on each):
--   1. clients.am_team_lead_id gets a real FK to users(id).
--   2. brief_revisions.brief_id, task_comments.author_id, and
--      task_attachments.uploaded_by get real FKs.
--   3. users.auth_id gets a UNIQUE constraint (now nullable too — see above).
--   4. campaigns.platform, social_insights.platform,
--      analytics_insights.platform, and kpi_scores.suggested_status get NO
--      CHECK constraint — their TS types end in `| string`, an intentional
--      escape hatch.
-- ============================================================================
...
-- ============================================================================
-- 0. DROP — the entire schema this file owns, plus every table/policy added
--    since v1. All confirmed empty; cascade is safe. Storage policies are
--    dropped by name (they don't cascade from a public-schema table drop).
--    Functions are dropped by every historical signature so this file is
--    re-runnable against a partially-migrated DB too, not just an empty one.
-- ============================================================================
...
create table public.users (
  ...
  -- Nullable (see header) — null marks a "pending" employee created via the
  -- Add Employee admin flow, before scripts/provisionAuthUsers.ts links a
  -- real Supabase Auth account out-of-band with the service-role key.
...
-- Added by 20260917 (employee provisioning): a real email-uniqueness
-- guarantee, needed once employees can be created from the browser (CSV
-- bulk upload especially) rather than only via a hand-curated seed list.
...
create table public.assignments (
  ...
  reason_notes text,
  -- Module 12 Phase 5: "New assignment" notification badge, cleared to null
  -- whenever agent_id changes, set when the assigned agent opens the
  -- client. Never gates access — mirrors clients.am_team_lead_viewed_at.
...
create table public.tasks (
  ...
  -- Module 12 Phase 5: same notification-badge concept as
  -- assignments.viewed_at, for programming_agent (which has no assignments
  -- row — task-based only). Generic column, cleared whenever assigned_to
  -- changes, set when the assignee opens the client the task belongs to.
  assignee_viewed_at timestamptz,
  -- Module 12 Phase 9: manually-pasted Drive URL. No real Drive API
  -- integration — same scaffolding-only posture as every other Module 6
  -- integration point. Generic, not creative-team-scoped at the DB level.
  drive_link text
);
...
-- ----------------------------------------------------------------------------
-- 1.11 client_contracts (Module 12 Phase 6)
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 1.14 analytics_insights (Module 6)
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 1.15 platform_connections (Module 6)
-- ----------------------------------------------------------------------------
...
create table public.daily_logs (
  ...
  -- Module 12 Phase 3: which client this entry's work relates to, if any.
  -- Single nullable id, matching every other work-artifact table in this
  -- schema — a day genuinely spanning multiple clients is already handled
  -- by writing multiple log rows. Filter/aggregation only: does NOT widen
  -- daily_logs_select_rls (still direct_report_visible(user_id) below) —
  -- the Team Activity tab surfaces task status only, never raw log text.
  client_id text references public.clients (id),
  created_at timestamptz not null default now()
);
...
-- Final version — see header. seo_team_lead's branch widened to admit
-- programming_agent (target-side), plus a new reciprocal branch so a
-- programming_agent can see their own peers and seo_team_lead.
...
-- Final (3-arg) version — see header. p_parent_task_id lets a subtask
-- inherit visibility from its parent (bounded by the 3-level nesting cap
-- enforced by enforce_nesting_depth() below). p_client_id (Module 12 Phase
-- 4) adds a narrow am_agent branch: visibility into every department's
-- tasks for a client they're the assigned am_agent for, via the same
-- client_am_agent_is_caller() helper reports/comparisons/campaigns already
-- use. Both trailing params default to null so 1- and 2-arg call sites
-- (task_comments/task_attachments RLS, storage policies) keep compiling
-- unchanged.
...
-- Final version — see header. Deliberately no same-role peer branch and no
-- agent-sees-their-lead reciprocal branch, unlike employee_visible():
-- personal/HR-sensitive tables only. seo_team_lead's branch widened to
-- admit programming_agent.
...
-- Added by 20260917 (employee provisioning): a narrow INSERT path for the
-- "Add Employee" admin screen, scoped to executive/head_of_technical only
-- (adding headcount is a leadership action, not a team-lead one — narrower
-- than users_update_capacity_rls's role set on purpose). `auth_id is null`
-- is load-bearing: it makes it structurally impossible for a browser
-- session to insert a row claiming any auth_id, real or fabricated — only
-- the service-role provisioning script (which bypasses RLS) ever sets one.
...
-- Final (widened) version — see header. The assigned agent (not just their
-- team lead) can now update their own row, so they can write viewed_at
-- themselves (Module 12 Phase 5). The app layer only ever sends
...
-- Final (3-arg) version — see header / task_visible()'s own comment.
...
-- ----------------------------------------------------------------------------
-- 4.11 client_contracts (Module 12 Phase 6)
-- ----------------------------------------------------------------------------
...
-- Final (narrowed) version — see header. Deliberately UNCHANGED by Module
-- 12 Phase 3's client_id column (filter/aggregation only, see 1.19 above).
...
-- Module 12 Phase 6: private bucket, folder-per-client (first path segment
-- is client_id, same convention as meeting-recordings above).
insert into storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
values (
  'client-contracts',
  'client-contracts',
